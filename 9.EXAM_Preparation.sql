...
--4
BEGIN TRANSACTION 
DECLARE @TouristesToDelete TABLE (Id INT)

INSERT INTO @TouristesToDelete(Id)
SELECT Id
FROM Tourists
WHERE [NAME] LIKE '%Smith%'

DELETE FROM Bookings
WHERE TouristId IN (SELECT Id FROM @TouristesToDelete)


DELETE FROM Tourists
WHERE Id IN (SELECT Id FROM @TouristesToDelete)
...
SELECT DATEDIFF(DAY, '2023-05-05', '2023-06-06')
--/Пример как се ползва 
...
CREATE PROC usp_SearchByCountry(@country VARCHAR(8)) 
AS
BEGIN
